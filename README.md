> [!NOTE]
> This repository is a detached fork of [`dwmkerr/terminal-ai`](https://github.com/dwmkerr/terminal-ai). It intentionally does not retain the upstream repository history.
...
- [Generating Shell Commands](#generating-shell-commands)
...
Use `ai generate` (or `ai gen`) to describe a task in plain English and get a ready-to-run shell command:

```bash
ai generate "list all docker containers sorted by size"
ai gen "find files modified in the last 24 hours larger than 10MB"
ai generate  # prompts you interactively if no description is given
```

Terminal AI sends the generated command to your terminal for review, then asks for confirmation before running it. Only your environment variable **names** (never values) are sent to the LLM, so secrets stay local.

```
$ ai generate "count lines of code in all TypeScript files"
...
> [!TIP]
> If you run `ai generate` without a TTY (e.g. in a pipe), the generated command is printed to stdout without any confirmation prompt, making it easy to inspect or pass to another tool.
...
The `ai generate` command (alias: `ai gen`) converts a natural language description into a shell command, shows it to you, and runs it after confirmation:

```bash
ai generate "archive all log files older than 7 days"
ai gen "show disk usage for each folder in the current directory"
```

Environment variable names (but never their values) are sent as context so the LLM knows which tools are available in your environment. The command runs with your current working directory and environment inherited.
