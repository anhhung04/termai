...
const cli = async (program: Command, executionContext: ExecutionContext) => {
  ...
  program
    .command("generate")
    .alias("gen")
    .description("Generate and run a shell command from natural language")
    .argument("[input]", "Natural language description of the command to run")
    .option(
      "-y, --yes",
      "Skip confirmation and run the generated command automatically",
    )
    .option(
      "-c, --copy",
      "Copy the generated command to clipboard instead of running it",
    )
    .action(async (input, options) => {
      await generate(executionContext, input, options.yes, options.copy);
    });
...
async function main() {
  ...
  try {
    ...
    //  Positional options: flags after a subcommand name are parsed by that
    //  subcommand, not the parent. This prevents -c/--copy on the parent chat
    //  command from shadowing the same flag on the generate subcommand.
